...
def render_table(sheet, rows, cols):
    ...
    for row in range(rows):
        ...
        for col_idx, col in enumerate(cols_list[1:], 1):
            ...
            def on_change(row=row, col=col_idx-1, cell_key=cell_key, show_formulas=show_formulas):
                ...
                if show_formulas:
                    st.session_state[cell_key] = cell.get("formula") or str(cell.get("computed") or "")
                else:
                    st.session_state[cell_key] = str(cell.get("computed") or "")
